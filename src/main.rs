...
fn main() {
    let order: String = String::from_utf8_lossy(&fs::read("./data/ord.tape").unwrap()).parse().unwrap(); // read the order tape
    let mut input = fs::read("./data/i.tape").unwrap().iter().map(|x|x-48).collect::<Vec<u8>>(); // read the input tape
    let split = order.split("\n").collect::<Vec<&str>>();
    let ord_split = split.iter().filter(|x|x.chars().nth(0).unwrap()!='#').collect::<Vec<&&str>>();

    let mut tms = vec![Machine{dist:[-1,-1],m:["E","E"],out:[3;2]};ord_split.len()];

    let _ = ord_split.iter().map(|x|{
    ...
    loop{
        ...
        match tmp.m[inp as usize]{
            ...
            "S" => {break},         // stop
            "E" => {println!("E");panic!();} // illegal state order
